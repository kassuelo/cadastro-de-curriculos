...
if(empty($nome)||empty($sobrenome)||empty($idade)||empty($escolaridade)||empty($area)||empty($idioma)||empty($cnh)){
	...
	$sql = "INSERT INTO curriculo (ID_pessoa, nome, sobrenome, idade, email, CPF, experiencias, escolaridade, area_atuacao, idiomas, cnh) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";


	if ($stmt = mysqli_prepare($id_conexao, $sql)) {

		/* bind parametros for markers */
		mysqli_stmt_bind_param($stmt, 'ississsssss', $ID_pessoa, $nome, $sobrenome, $idade, $email, $cpf, $experiencias, $escolaridade, $area, $idioma, $cnh);
